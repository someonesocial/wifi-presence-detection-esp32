/*
 * NodeMCU V3 (ESP8266MOD) Sensor Dashboard - Arduino IDE
...
// HTML als einzelne Zeile - kompakt fuer ESP8266 RAM
const char index_html[] PROGMEM = 
"<!DOCTYPE html><html lang='de'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1.0'>"
"<title>NodeMCU V3 Dashboard</title><script src='https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js'></script>"
"<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',sans-serif;background:linear-gradient(135deg,#1a1a2e,#16213e);color:#e0e0e0;min-height:100vh;padding:20px}"
"h1{text-align:center;margin-bottom:10px;color:#00d4ff;font-size:2rem}.subtitle{text-align:center;color:#888;margin-bottom:25px;font-size:.9rem}"
...
".card h3{font-size:.9rem;color:#aaa;margin-bottom:15px}.current-value{font-size:2rem;font-weight:bold;color:#fff;margin-bottom:10px}"
".current-value .unit{font-size:1rem;color:#888;font-weight:normal}canvas{max-height:200px}"
".footer{text-align:center;margin-top:30px;color:#555;font-size:.8rem}"
"@media(max-width:600px){h1{font-size:1.4rem}.grid{grid-template-columns:1fr}}</style></head><body>"
"<h1>NodeMCU V3 Sensor Dashboard</h1><p class='subtitle'>ESP8266MOD | Echtzeit-Umgebungsmonitoring & WLAN-Prasenzerkennung</p>"
"<div class='status-bar'><div class='status-item'><span class='label'>Verbindung:</span><span class='value' id='connStatus'>Lade...</span></div>"
"<div class='status-item'><span class='label'>RSSI:</span><span class='value' id='rssiDisplay'>-- dBm</span></div>"
"<div class='status-item'><span class='label'>Prasenz:</span><span class='value' id='presenceDisplay'>Kalibrierung...</span><span id='presenceIndicator'></span></div></div>"
"<div class='grid'><div class='card'><h3>Raumtemperatur (AM2302)</h3><div class='current-value' id='valRoomTemp'>--<span class='unit'> C</span></div><canvas id='chartRoomTemp'></canvas></div>"
...
"<div class='card'><h3>WiFi Signalqualitat</h3><div class='current-value' id='valQuality'>--<span class='unit'> %</span></div><canvas id='chartQuality'></canvas></div></div>"
...
"const charts={roomTemp:new Chart(document.getElementById('chartRoomTemp'),chartConfig('C','#ff6b6b',10,40)),"
...
"const MAX_POINTS=50;function updateChart(ch,v){const t=new Date();const tm=t.getHours().toString().padStart(2,'0')+':'+t.getMinutes().toString().padStart(2,'0')+':'+t.getSeconds().toString().padStart(2,'0');ch.data.labels.push(tm);ch.data.datasets[0].data.push(v);if(ch.data.labels.length>MAX_POINTS){ch.data.labels.shift();ch.data.datasets[0].data.shift();}ch.update('none');}"
"function rssiToQuality(r){if(r>=-50)return 100;if(r<=-100)return 0;return 2*(r+100);}"
"const cs=document.getElementById('connStatus');const pi=document.getElementById('presenceIndicator');const pd=document.getElementById('presenceDisplay');const rd=document.getElementById('rssiDisplay');let lastDataTime=0;"
"function updateData(){fetch('/api/data').then(r=>r.json()).then(d=>{cs.textContent='Verbunden';cs.style.color='#0f8';lastDataTime=Date.now();"
...
"rd.textContent=d.rssi+' dBm';if(d.calibrated){if(d.presence){pd.textContent='ERKANNT';pd.style.color='#0f8';pi.classList.add('active');}else{pd.textContent='Keine Bewegung';pd.style.color='#888';pi.classList.remove('active');}}else{pd.textContent='Kalibrierung... ('+d.calProgress+'%)';pd.style.color='#ffe66d';}"
...
"}).catch(e=>{cs.textContent='Fehler';cs.style.color='#f66';console.error('Fetch Fehler:',e);});}"
"setInterval(updateData,2000);updateData();"
"setInterval(function(){if(Date.now()-lastDataTime>5000){cs.textContent='Getrennt';cs.style.color='#f66';}},3000);"
"</script></body></html>";
...
void readSensors() {
    ...
    if (!isnan(h) && !isnan(t)) { humidity = h; roomTemp = t; }
...
void setup() {
    ...
    Serial.println("(ESP8266MOD) - AJAX Polling");
    dht.begin();
    WiFi.mode(WIFI_STA);
